...
const checkpointFile = 'checkpoint.json'; // Checkpoint file to save progress
const missingFilesLog = 'missing_files.log'; // Log missing files
...
// Log missing files to a separate file
async function logMissingFile(channelNumber) {
    await fs.appendFile(missingFilesLog, `Missing file for channel ${channelNumber}\n`);
}
...
// Update the grouped channels file
async function updateGroupedChannelsFile(groups) {
    const resultFile = 'grouped_channels.json';
    await fs.writeFile(resultFile, JSON.stringify(groups, null, 2)); // Async write
    console.log(`Updated Image Groups written to ${resultFile}`);
}
...
async function processImages(channels) {
    ...
    // Save final results
    await updateGroupedChannelsFile(groups);
