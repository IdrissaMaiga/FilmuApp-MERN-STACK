...
const extractM3u8UrlsByLogo = async (inputFile, outputFile) => {
  try {
    ...
    for (const [logo, channels] of logoMap.entries()) {
      ...
      const logoName = [...nameCounts.entries()].reduce((a, b) =>
        b[1] > a[1] ? b : a
      )[0]
      ...
      channels.forEach(channel => {
        outputData.push(
          `${
            channel.url
          }, tvg-name="${channel.cleanedTvgName.toUpperCase()}" className="${channel.className.toUpperCase()}" resolution="${
            channel.resolution
          }" uct="${channel.uct.toUpperCase()}"`
        )
      })
...
const outputFilePath = path.join(__dirname, 'outputfilter1.txt')
