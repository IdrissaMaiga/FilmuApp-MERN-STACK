...
const extractM3u8UrlsByLogo = async (inputFile, outputFile) => {
  try {
    ...
    const outputData = [];
    for (const [logoName, { logos, channels }] of groupedChannels.entries()) {
      const logoUrls = logos.join(', ');
      outputData.push(`LogoName: ${logoName}, Logos: ${logoUrls}`); // Display logoName and logos
      channels.forEach(channel => {
        outputData.push(
          `${channel.url}, tvg-name="${channel.cleanedTvgName.toUpperCase()}" className="${channel.className.toUpperCase()}" resolution="${channel.resolution}" uct="${channel.uct.toUpperCase()}"`
        );
      });
...
const outputFilePath = path.join(__dirname, 'filtered.txt');
