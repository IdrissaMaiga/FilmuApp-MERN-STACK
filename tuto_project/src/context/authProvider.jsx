...
async function getDeviceInfo() {
  ...
  const flagdevice = async (data) => {
    ...
    try {
      const response = await api.patch("/api/user/flag",data, { withCredentials: true });
  ...
  const login = async (data) => {
    ...
    try {
      ...
      const response = await api.post('/api/auth/login', {...data,deviceInfo}, { withCredentials: true });
      setUser(response.data);
      toast({
        title: "Logged in",
