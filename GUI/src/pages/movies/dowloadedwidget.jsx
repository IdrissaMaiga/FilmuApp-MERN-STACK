...
import { Box, Flex, IconButton, Skeleton,  useBreakpointValue, useColorModeValue } from "@chakra-ui/react";
...
const DdWidget = () => {
  ...
  useEffect(() => {
    ...
    user.downloads.map(d=>{
        if(d?.SerieId)seriesIds.push(d.SerieId)
        if(d?.movieId)movieIds.push(d.movieId)
    ...
    fetchWatchingData({movieIds,seriesIds})
    .then((res) => {
        setMovies(res.data|| []);
