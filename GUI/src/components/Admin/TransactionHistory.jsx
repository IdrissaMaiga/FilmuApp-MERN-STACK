...
import Moneyflow from './money';

const TransactionHistory = () => {
  const { currency } = useAuth();
  const { getAllTransactions, transactions, loading: tloading } = useAdmin();
  ...
  return (
    <Tabs variant="soft-rounded" colorScheme="teal" p={2}>
      <TabList m={0} p={0}>
        ...
        <Tab>Moneyflow</Tab> {/* New tab for statistics */}
        <Tab>Statistics</Tab> {/* New tab for statistics */}
      </TabList>
      ...
      <TabPanels m={0} p={2}>
            ...
            <Box p={2} m={0} bg={bg} color={textColor} mt="3">
              <Flex mb={4} alignItems="center">
                ...
                <Flex alignItems="center">
                  ...
                  <Switch colorScheme='teal' isChecked={showImages} onChange={() => setShowImages(!showImages)} />
                </Flex>
              </Flex>
                  ...
                  {filteredTransactions.length ? (
                    <Grid templateColumns={{ base: '1fr', md: 'repeat(3, 1fr)', lg: 'repeat(5, 1fr)' }} gap={4}>
                      {filteredTransactions.map((transaction) => (
                        <Box key={transaction.id} p={4} bg={getColor(transaction)} boxShadow="sm" color="black">
        ...
        <TabPanel mt={2} p={0}>
          <StaticComponent transactions={transactions} /> {/* Include StaticComponent */}
        </TabPanel>
